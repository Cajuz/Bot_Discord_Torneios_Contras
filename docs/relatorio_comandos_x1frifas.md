...
1. **`/x1`** (MatchCog) — cria a partida com status `criado`.
2. **Mediador entra na thread** (automático via fila) — status passa para `aguardando_partida`.
3. **`/sala`** (MediatorCog) — mediador inicia oficialmente a partida → `partida_iniciada`.
4. **`!wt red`** ou **`!wt blue`** (MatchCog) — declara vencedor → `finalizado`.
