...
class PixLog:
    ...
    async def send_pix_log(self, interaction: discord.Interaction, pix_antigo: str | None, pix_novo: str):
        ...
        if pix_antigo:
            embed.add_field(name="🔴 PIX Antigo",value=f"`{pix_antigo}`",inline=False)
        embed.add_field(name="🟢 PIX Novo",value=f"`{pix_novo}`",inline=False)
        embed.set_footer(text="Sistema de Logs • PIX")
        await canal.send(embed=embed)

def build_pix_log_embed():
    embed = discord.Embed(title="💳 Sistema de Logs de PIX",
        description=(
