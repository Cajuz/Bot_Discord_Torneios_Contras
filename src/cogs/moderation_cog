...
def _is_adm():
    """Check: apenas ADM ou administrador do servidor."""
...
async def _log_action(
    ...
    """Envia embed de log para #logs-command."""
...
class ModerationCog(commands.Cog, name="Moderação"):
    ...
    async def aviso(
        ...
        if not ch:
            await interaction.response.send_message(
                f"❌ Canal `#{AVISOS_CHANNEL}` não encontrado.", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"✅ Aviso postado em {ch.mention}.", ephemeral=True
        )
        await _log_action(
            interaction.guild, interaction.user,
            "Aviso", ch.mention, titulo,
        )
    ...
    async def ban(
        ...
        deletar_mensagens: app_commands.Range[int, 0, 7] = 0,
    ):
        ...
        try:
            ...
            await _log_action(
                interaction.guild, interaction.user,
                "Ban", f"{membro} (`{membro.id}`)", motivo, COLOR_ERROR,
            )
    ...
    async def unban(
        ...
        try:
            ...
            embed = discord.Embed(
                title="✅ Ban Removido",
                description=f"**{user}** (`{uid}`) foi desbanido.",
                color=COLOR_SUCCESS,
                timestamp=utcnow(),
            )
            embed.add_field(name="Motivo", value=motivo, inline=False)
            ...
            await _log_action(
                interaction.guild, interaction.user,
                "Unban", f"{user} (`{uid}`)", motivo, COLOR_SUCCESS,
            )
    ...
    async def kick(
        ...
        if membro.top_role >= interaction.user.top_role:
            await interaction.response.send_message(
                "❌ Você não pode expulsar alguém com cargo igual ou superior ao seu.", ephemeral=True
            )
            return
        try:
            ...
            await _log_action(
                interaction.guild, interaction.user,
                "Kick", f"{membro} (`{membro.id}`)", motivo, COLOR_WARN,
            )
        except discord.Forbidden:
    ...
    async def timeout(
        ...
        if membro.top_role >= interaction.user.top_role:
            await interaction.response.send_message(
                "❌ Você não pode silenciar alguém com cargo igual ou superior ao seu.", ephemeral=True
            )
            return
        try:
            ...
            await _log_action(
                interaction.guild, interaction.user,
                f"Timeout ({duracao})", f"{membro} (`{membro.id}`)", motivo, COLOR_WARN,
            )
        except discord.Forbidden:
    ...
    @app_commands.describe(
        membro="Membro para remover o timeout",
        motivo="Motivo",
    )
    @_is_adm()
    async def untimeout(
        ...
        try:
            ...
            await interaction.response.send_message(
                f"✅ Timeout de {membro.mention} removido.", ephemeral=True
            )
            await _log_action(
                interaction.guild, interaction.user,
                "Untimeout", f"{membro} (`{membro.id}`)", motivo, COLOR_SUCCESS,
            )
        except discord.Forbidden:
    ...
    # ─────────────────────────────────────────
    # /silence — muda cargo para Bloqueado
    # ─────────────────────────────────────────

    @app_commands.command(name="silence", description="Bloqueia um membro via cargo (anti-spam).")
    @app_commands.describe(
        membro="Membro a ser bloqueado",
    ...
    async def silence(
        ...
        interaction: discord.Interaction,
        membro: discord.Member,
        motivo: str = "Bloqueado manualmente",
    ):
        try:
            ...
            await interaction.response.send_message(
                f"🔇 {membro.mention} silenciado. Motivo: *{motivo}*", ephemeral=False
            )
            await _log_action(
                interaction.guild, interaction.user,
                "Silence", f"{membro} (`{membro.id}`)", motivo, COLOR_WARN,
            )
        except Exception as e:
    ...
    async def bloquear(
        ...
        try:
            ...
            await interaction.response.send_message(
                f"✅ {membro.mention} bloqueado. Motivo: *{motivo}*"
            )
            await _log_action(
                interaction.guild, interaction.user,
                "Bloquear", f"{membro} (`{membro.id}`)", motivo, COLOR_WARN,
            )
        except Exception as e:
    ...
    async def desbloquear(
        ...
        try:
            ...
            await _log_action(
                interaction.guild, interaction.user,
                "Desbloquear", f"{membro} (`{membro.id}`)", "—", COLOR_SUCCESS,
            )
        except Exception as e:
    ...
    async def warn(
        ...
        try:
            ...
            await _log_action(
                interaction.guild, interaction.user,
                f"Warn (total: {total})", f"{membro} (`{membro.id}`)", motivo, COLOR_WARN,
            )
        except Exception as e:
    ...
    @app_commands.describe(
        ...
        membro="Apagar apenas mensagens deste membro (opcional)",
    )
    @_is_adm()
    async def limpar(
        self,
        ...
        try:
            ...
            await interaction.followup.send(
                f"🧹 {len(deleted)} mensagem(ns) removida(s)" +
                (f" de {membro.mention}" if membro else "") + ".",
                ephemeral=True,
            )
            await _log_action(
                interaction.guild, interaction.user,
                "Limpar", interaction.channel.mention,
                f"{len(deleted)} msgs" + (f" de {membro}" if membro else ""),
                COLOR_WARN,
            )
        except discord.Forbidden:
