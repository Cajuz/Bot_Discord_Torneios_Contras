...
class AntiSpamService:
    ...
    Desbloquear = remove Bloqueado + devolve Membro (só ADM)
    Card postado em #membros-bloqueados e atualizado ao desbloquear.
    """

    def __init__(self, bot: discord.Client):
        self.bot = bot
        self._rate: Dict[int, Dict[int, Deque[float]]] = defaultdict(
            lambda: defaultdict(deque))
        self._dup:  Dict[int, Dict[int, _DupState]]    = defaultdict(
            lambda: defaultdict(_DupState))
                ...
                try:
                    ...
                    if block_role not in member.roles:
                        await member.add_roles(
                            block_role, reason="AntiSpam: restaurar bloqueio")
                        count += 1
                    if member_role and member_role in member.roles:
                        await member.remove_roles(
                            member_role, reason="AntiSpam: restaurar bloqueio")
                except Exception:
                    continue

            if count:
                logger.info(
                    f"[AntiSpam] {count} membros bloqueados restaurados em {guild.name}")
    ...
    async def handle_member_join(self, member: discord.Member):
        try:
            ...
            if doc and doc.get("spam_blocked"):
                ...
                if block_role not in member.roles:
                    await member.add_roles(
                        block_role, reason="AntiSpam: membro bloqueado re-entrou")
                if member_role and member_role in member.roles:
                    await member.remove_roles(
                        member_role, reason="AntiSpam: membro bloqueado re-entrou")
                logger.warning(
                    f"[AntiSpam] {member.name} re-entrou bloqueado — cargo reaplicado")
        except Exception as e:
    ...
    async def process_message(self, message: discord.Message):
        ...
        if content_key:
            ...
            if (ds.content_key != content_key
                    or (now - ds.first_ts) > AntiSpamConfig.DUP_WINDOW_SECS):
    ...
    async def _block_member(
        self, member: discord.Member, reason: str, evidence: str = ""
    ):
        guild       = member.guild
        block_role  = await self._get_block_role(guild)
        member_role = await self._get_member_role(guild)
    ...
    async def _post_block_card(
        ...
        try:
            ...
            if not channel:
                logger.warning(
                    f"[AntiSpam] Canal '{AntiSpamConfig.SPAM_BLOCKED_CHANNEL_NAME}' não encontrado")
                return None
    ...
    async def unblock_member(
        ...
        if member:
            try:
                if block_role in member.roles:
                    await member.remove_roles(
                        block_role, reason=f"AntiSpam: desbloqueado por {by}")
            except Exception as e:
                logger.error(
                    f"[AntiSpam] Erro ao remover Bloqueado de {member_name}: {e}")
                raise

            try:
                if member_role and member_role not in member.roles:
                    await member.add_roles(
                        member_role, reason=f"AntiSpam: acesso restaurado por {by}")
            except Exception as e:
                logger.warning(
                    f"[AntiSpam] Não consegui devolver Membro para {member_name}: {e}")
        ...
        if card_message:
            try:
                ...
                await card_message.edit(
                    embed=new_embed, view=SpamBlockCardView(disabled=True))
            except Exception as e:
...
anti_spam_service: Optional[AntiSpamService] = None


def init_anti_spam_service(bot: discord.Client) -> AntiSpamService:
    global anti_spam_service
