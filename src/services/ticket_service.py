...
class TicketService:
    ...
    async def create_ticket(
        self, discord_id: str, username: str, guild_id: int, categoria: str, descricao: str
        ...
        doc = Ticket.create_document(
            ...
            ticket_id=ticket_id, categoria=categoria, descricao=descricao
        )
        await col.insert_one(doc)
        return True, "", Ticket.from_dict(doc)

    async def assume_ticket(
        self, ticket_id: str, atendente_id: str, guild_id: int
    ) -> tuple[bool, str, Optional[Ticket]]:
        col = db.get_collection("tickets")
        ...
        await col.update_one(
            {"ticket_id": ticket_id},
            {"$set": {"status": Ticket.STATUS_PENDENTE, "atendente_id": str(atendente_id), "updated_at": now}}
        )
    ...
    async def close_ticket(
        ...
        await col.update_one(
            {"ticket_id": ticket_id},
            {"$set": {"status": Ticket.STATUS_FECHADO, "closed_at": now, "updated_at": now}}
        )
